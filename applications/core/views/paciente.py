...
@csrf_exempt
@require_POST
def paciente_create_ajax(request):
    import json
    from applications.core.models import Paciente
    ...
    cedula = data.get('cedula_ecuatoriana', '').strip()
    ...
    # Validación básica
    if not (nombres and apellidos and cedula and fecha_nacimiento and telefono and sexo and estado_civil and direccion):
    ...
    if Paciente.objects.filter(cedula_ecuatoriana=cedula).exists():
        return JsonResponse({'success': False, 'error': 'Ya existe un paciente con esa cédula.'})

    # Crear paciente
    paciente = Paciente.objects.create(
        nombres=nombres,
        apellidos=apellidos,
        cedula_ecuatoriana=cedula,
    ...
    return JsonResponse({'success': True, 'paciente': {
        'id': paciente.id,
        'nombres': paciente.nombres,
        'apellidos': paciente.apellidos,
        'cedula_ecuatoriana': paciente.cedula_ecuatoriana,
